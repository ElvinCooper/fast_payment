...
def test_login_success(client, mock_db_conn, mock_pg_conn):
    ...
    # Mock para get_user_database y get_user_type
    with (
        patch("app.routers.auth.get_user_database") as mock_get_db,
        patch("app.routers.auth.get_user_type") as mock_get_type,
    ):
...
def test_login_failure(client, mock_db_conn, mock_pg_conn):
    ...
    with (
        ...
        patch("app.routers.auth.get_user_type", return_value=None),
    ):
        response = client.post(
            "/api/v1/auth/login",
